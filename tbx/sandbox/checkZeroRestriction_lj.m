function [status, Zcell, Msg]=checkZeroRestriction_lj(meta, exactZerosTbx)
...
count=0;
for ii=1:size(exactZerosTbx,2)
    restablecat=cat(2,exactZerosTbx{:,ii});
...
if count>0 % if we found something in the table then the sign res routine is activated
    ...
    for ii=1:numShock % loop over shocks
        ...
        numzerores=sum(exactZerosTbx{:,ii}==0);
        ...
        elseif numzerores>0
            ...
            Zcell{ii} = double(exactZerosTbx{:,ii}==0)';
