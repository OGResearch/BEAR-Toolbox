...
for iteration=1:numt % beginning of forecasting loop
  if window_size>0
    ...
    % get sampler
    outSampler = lj_panel_factor_dynamic_smpl(data_endo,data_exo,const,lags,It,Bu,cband,alpha0,delta0,pick,pickf,rho,gamma,a0,b0,psi);
    ...
    Fperiods  = 10;
    % iSample = 50;
    [beta_draw] = lj_panel_factor_dynamic_drawer(smpl, thetabar, Xi, rho, gamma, Flocation, Fperiods);

    [Y, X] = lj_get_XY_format(data_endo,const,lags);
    lj_panel_factor_dyamic_simulate()
