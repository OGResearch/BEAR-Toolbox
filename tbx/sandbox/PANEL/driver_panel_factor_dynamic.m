...
run("driver_init.m");
% it is too slow, so I am decreasing number of draws havily
It = 200; % orig 2000
Bu = 100; % orig 1000
...
for iteration=1:numt % beginning of forecasting loop
  if window_size>0
    ...
    [theta_median,theta_std,theta_lbound,theta_ubound,sigma_median,Ymat,y, Xtilde,theta_gibbs,sigma_gibbs,sigmatilde_gibbs,Zeta_gibbs,phi_gibbs,B_gibbs,Xi,thetabar,N,n,m,p,T,d,k] = driver_estimation_panel_factor_dynamic(data_endo,data_exo,const,lags,It,Bu,cband,alpha0,delta0,pick,pickf,rho,gamma,a0,b0,psi);
    ...
    %% BLOCK 3: FORECASTS
    %%%%%%%%%%
    %%%%% Skip this block as there is an error. Reason - they call function normrnd with only two arguments, while 4 are expected. But I can't figure out if this is the one that should be used. 
    %%%%%%%%%%
    ...
    % not all block can be displayed, as there are no forecast results
