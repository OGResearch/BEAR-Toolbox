...
function [fcastTbl, contribsTbl] = conditionalForecast(this, fcastSpan, options)
    ...
    for i = 1 : numPresampled
        ...
        for j = 1 : numUnits
            if ~isempty(cfconds)
                internalOptions.cfconds = cfconds(:, :, j);
            end
            if ~isempty(cfshocks)
                internalOptions.cfshocks = cfshocks(:, :, j);
                for k = 1 : numel(internalOptions.cfshocks)
                    internalOptions.cfshocks{k} = internalOptions.cfshocks{k} - (j-1)*numShockConcepts;
                end
            end
            if ~isempty(cfblocks)
                internalOptions.cfblocks = cfblocks(:, :, j);
            end
            unitInitY = initY(:, :, j);
            unitD = sample.D(:, :, j);
            if numUnits == 1
                ...
                unitBeta = meta.extractUnitFromCells(draw.beta, j, dim=3);
            end
            %
            %
